...
class Model_Pluggs_Imagick {
	...
	function generateView($img, $width = WIDTH_VIEW, $height = HEIGHT_VIEW, $quality = 70){
		...
		if(is_file($img)){
			...
			if(file_put_contents(URL_IMG_ALUNO.$filename_no_ext.FORMATO_THUMBS, $imagick) === false) {
	...
	function generateThumbnail($img, $width = WIDTH_THUMB, $height = HEIGHT_THUMB, $quality = 90){
		...
		if(is_file($img)){
			...
			if(file_put_contents(URL_IMG_ALUNO_THUMBS.$filename_no_ext.SUBNOME_THUMBS.FORMATO_THUMBS, $imagick) === false) {
